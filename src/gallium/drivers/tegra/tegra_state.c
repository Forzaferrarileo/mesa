...
static void tegra_set_scissor_state(struct pipe_context *pcontext,
				    const struct pipe_scissor_state *scissor)
{
	fprintf(stdout, "> %s(pcontext=%p, scissor=%p)\n", __func__, pcontext,
		scissor);
	fprintf(stdout, "< %s()\n", __func__);
}
...
static void tegra_set_fragment_sampler_views(struct pipe_context *pcontext,
					     unsigned int count,
					     struct pipe_sampler_view **views)
{
	fprintf(stdout, "> %s(pcontext=%p, count=%u, views=%p)\n", __func__,
		pcontext, count, views);
	fprintf(stdout, "< %s()\n", __func__);
}

static void tegra_set_vertex_sampler_views(struct pipe_context *pcontext,
					   unsigned int count,
					   struct pipe_sampler_view **views)
{
	fprintf(stdout, "> %s(pcontext=%p, count=%u, views=%p)\n", __func__,
...
void tegra_context_state_init(struct pipe_context *pcontext)
{
	...
	pcontext->set_scissor_state = tegra_set_scissor_state;
	pcontext->set_viewport_state = tegra_set_viewport_state;
	pcontext->set_fragment_sampler_views = tegra_set_fragment_sampler_views;
	pcontext->set_vertex_sampler_views = tegra_set_vertex_sampler_views;
...
static void tegra_bind_fragment_sampler_states(struct pipe_context *pcontext,
					       unsigned int count, void **so)
{
	fprintf(stdout, "> %s(pcontext=%p, count=%u, so=%p)\n", __func__,
		pcontext, count, so);
	fprintf(stdout, "< %s()\n", __func__);
}

static void tegra_bind_vertex_sampler_states(struct pipe_context *pcontext,
					     unsigned int count, void **so)
{
	fprintf(stdout, "> %s(pcontext=%p, count=%u, so=%p)\n", __func__,
		pcontext, count, so);
...
void tegra_context_sampler_init(struct pipe_context *pcontext)
{
	...
	pcontext->bind_fragment_sampler_states = tegra_bind_fragment_sampler_states;
	pcontext->bind_vertex_sampler_states = tegra_bind_vertex_sampler_states;
